...
export const getSubCategoryOfProduct = async (req: Request<{ productId: string }>, res: Response): Promise<void> => {
  try {
    const product = await ProductSubCategory.findAll({where:{categoryId:req.params.productId}})
    if (!product) {
      res.status(404).json({ message: 'product subcategories not found' });
      return;
    }
    res.status(200).json(product);
  } catch (error) {
